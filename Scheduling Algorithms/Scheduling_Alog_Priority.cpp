...
class Process{
	...
	public: void getdata(){
        ...
        cout<<"Enter the Priority for this process : ";
        cin>>priority;
	}
	public : void display(){
		cout<<"Process "<<process_name<<" : "<<Arrival_time<<cout.width(10)<<Burst_time<<cout.width(10)<<priority<<cout.width(10)<<Completion_time<<cout.width(10)<<Turn_Around_time<<cout.width(10)<<Waiting_time<<endl;
	}
...
class Scheduling{
	...
	public : void displayProcess(){
		cout<<"Process Name AT"<<cout.width(9)<<"BT"<<cout.width(9)<<"CT"<<cout.width(10)<<"P"<<cout.width(10)<<"TAT"<<cout.width(10)<<"WT"<<endl;
			for(int i =0;i<n;i++){
				processes[i].display();
			}
	}
	...
	public : void completiontime(){
		...
		for(int i = 1;i<n;i++){
			...
			int low = processes[i].priority;
			for(int j = i;j<n;j++){
				if(temp>=processes[j].Arrival_time && low >= processes[j].priority){
					low = processes[j].priority;
					val = j;
				}
			}
